...
def batch_interpolate(conts, n_int=100):
    
#    A = np.tile(numpy_bezier_matrix(n_control=conts.shape[1],n_int=100),(conts.shape[0],1,1))
    A = numpy_bezier_matrix(n_control=conts.shape[1],n_int=n_int)
    
    return np.transpose(np.squeeze(np.dot(A,conts),axis=0),(1,0,2))

    

def numpy_bezier_matrix(n_control=5,n_int=100):    

...
#            
        ...
        if iteration % config.vis_interval == 0:
            ...
            base_postprocessed = postprocessors['bbox'](threshed_outputs,torch.Tensor(np.tile(np.expand_dims(np.array(config.patch_size),axis=0),[seq_images.shape[0],1])).cuda())
...
#            temp_dict['poly_list'] = b['blob_ids'].cuda()
        ...
        match_static_indices,  match_poly_indices = criterion.matcher(outputs, cuda_targets, do_obj=False, do_polygons=False)
...
def log_results(ious, summary, split, epoch):
     temp_list=[]
     for seq in range(len(ious)):
         for obj in range(len(ious[seq])):
             temp_list.append(ious[seq][obj])        
             summary.add_scalar('val/seq_'+str(seq)+'_obj_'+str(obj), ious[seq][obj], epoch)
     summary.add_scalar('val/mean_iou', np.mean(temp_list))
     logging.error('VAL EPOCH ' + str(epoch) + ', '+str(np.mean(temp_list)))
...
def main():
   ...
   # base_dir+'/'+model_name
    ...
    optimizer = torch.optim.AdamW(param_dicts, lr = 1e-05,
    ...
    if config.load_pretrained_backbone:
        load_pretrained_backbone(config.backbone_ckpt_path, model.backbone.backbone_net, optimizer, lr_scheduler, True)
        epoch, best_iou, iteration = 1,0,0
       
    else:
       
        epoch, best_iou, iteration = load_checkpoint(os.path.join('/scratch_net/catweazle/cany/simplice/checkpoints/trans-mc', 'MC-TR.pth'),

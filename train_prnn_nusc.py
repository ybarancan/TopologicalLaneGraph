...
#            Visualise
        ...
        # if iteration % config.vis_interval == 0:
            ...
            #                               poly_indices ,metric_visuals=metric_visuals, gt_train=True)
        
        
  
            
            
        #     visualise(summary, seq_images.cpu().numpy()[0], maxed_masks.cpu().numpy()[0],pred, iteration, 'train',config)
...
#            vis_tools.save_results_eval(seq_images.cpu().numpy(), outputs,targets,  config)
        ...
        # static_metrics, object_metrics = temp_con.get_res_dict
            
            
            # if ((targets[0]['sample_token'] == 'd68b0e57615c46939a61a774c5967a26') | (targets[0]['sample_token'] == '9b3ff4bc3d1d46a19ddae004ec9c0ae5') |
            #         (targets[0]['sample_token'] == '18dfa0ff351649f5a76cc7025507304d')):
                
            
            #      static_res_dict, object_res_dict = temp_con.get_res_dict
...
#
#        match_indices = criterion.matcher(outputs, cuda_targets)
#        matched_outputs = criterion.get_assoc_estimates( outputs, match_indices)
#        
#        inter_dict, idx, target_ids = criterion.get_interpolated(matched_outputs, cuda_targets, match_indices)
#        inter_points = inter_dict['interpolated'].detach().cpu()
#        confusion.update(inter_points, idx, targets[0],target_ids)
#        
#        
#        outputs = model.thresh_and_assoc_estimates(outputs,thresh=0.5)
#        base_postprocessed = postprocessors['bbox'](outputs,torch.Tensor(np.tile(np.expand_dims(np.array(config.patch_size),axis=0),[seq_images.shape[0],1])).cuda())
#        # logging.error('PREDS ' )
#        # logging.error(str(base_postprocessed[0]['probs']))
#        
...
def main():
    ...
    parser.add_argument('--resume', default= '/cluster/work/cvl/cany/TPLR/prnn', 
                        help='path to an experiment to resume')
    
    parser.add_argument('--exp', default='/cluster/home/cany/TPLR/baseline/Experiments/mle.json', 
                        help='path to an experiment to resume')
    ...
    # Setup experiment
    model, criterion, postprocessors = build(args, config,opts, old=True)
    
    model.to(device)
    
    train_loader,train_dataset, val_loader, val_dataset = data_factory.build_nuscenes_dataloader(config,args,polyline=True)
...
#        
        ...
        epoch, best_iou, iteration = load_checkpoint(os.path.join(base_dir,'final_ckpts','PRNN_MC_Nusc.pth'),  model, optimizer, lr_scheduler)
